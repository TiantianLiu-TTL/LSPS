...
def load_data():
    edge_index = pd.read_csv('./data/web-EPA/Edge_4253.txt', sep=' ').iloc[:,:2].T
    print('edge_index samples:', edge_index[:5])
    edge_weights = pd.read_csv('./data/web-EPA/whole_edge_dist.csv', sep=',', header=0)
    hop_edge_dist = pd.read_csv('./data/web-EPA/whole_edge_hop_dist.csv', sep=',', header=0).astype(float)

    node_feats = pd.read_csv('./data/web-EPA/feature_2hop2tier.txt', sep='\t',header=None, index_col=0)
    node_feats = sc.fit_transform(node_feats)
    # print(node_feats[:5])
    return edge_index, edge_weights, node_feats, hop_edge_dist
