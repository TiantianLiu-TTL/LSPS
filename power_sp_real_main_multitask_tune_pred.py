...
model_path = './path/power'
pred_path = './pred/power'
...
def load_data():
    zero_edge_index = pd.read_csv('./data/inf-power/edge_3hop2tier_tier_0.txt', sep='\t', names=['s', 't']).T.astype(int)
    one_edge_index = pd.read_csv('./data/inf-power/edge_3hop2tier_tier_1.txt', sep='\t', names=['s', 't']).T.astype(int)
    two_edge_index = pd.read_csv('./data/inf-power/edge_3hop2tier_tier_2.txt', sep='\t', names=['s', 't']).T
    edge_weights = pd.read_csv('./data/inf-power/whole_edge_dist.csv', sep=',', header=0).astype(float)
    hop_edge_dist = pd.read_csv('./data/inf-power/whole_edge_hop_dist.csv', sep=',', header=0).astype(float)
    node_feats = pd.read_csv('./data/inf-power/feature_3hop2tier.txt', sep='\t',header=None, index_col=0).astype(float)
...
with torch.no_grad():
    ...
    weight_df.to_csv(os.path.join(pred_path, 'weight_dist.csv'), sep=',', header=True,
                     index=False)
    hop_df.to_csv(os.path.join(pred_path, 'hop_dist.csv'), sep=',', header=True, index=False)

    # weight_df.to_csv(os.path.join(pred_path, 'weight_dist_{flag}.csv'.format(flag=flag)), sep=',', header=True, index=False)
    # hop_df.to_csv(os.path.join(pred_path, 'hop_dist_{flag}.csv'.format(flag=flag)), sep=',', header=True, index=False)
