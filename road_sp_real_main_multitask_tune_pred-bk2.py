...
model_path = './path/road'

pred_path = f'./pred/road/node{args.node}'
...
def load_data():
    ...
    distance = pd.read_csv(f'./data/road-NA/node{args.node}/samples_weight_{args.node}.txt', sep=' ', names=['s', 't', 'weight', 'hop'])
...
# if edge_weights.shape[0]%batch_size == 0:
#     num_all_batch = int(edge_weights.shape[0]/batch_size)
# else:
#     num_all_batch = int(edge_weights.shape[0]/batch_size) + 1
...
for pre_epoch in range(pretrain_epochs):
    ...
    for batch in range(num_train_batch):
        ...
        print('{n} epoch {b} batch train weight pred shape'.format(n=pre_epoch, b=str(batch)), pred.shape)
        print('{n} epoch {b} batch train weight pred'.format(n=pre_epoch, b=str(batch)), pred[:10])
        print('{n} epoch {b} batch train weight true:'.format(n=pre_epoch, b=str(batch)), train_weights[batch * batch_size:(batch + 1) * batch_size][:10])
        ...
        train_weight_pred.append(pred_df)
        train_hop_pred.append(hop_pred_df)
    ...
    train_weight_pred_df = pd.concat(train_weight_pred, axis=0).reset_index(drop=True)
    train_hop_pred_df = pd.concat(train_hop_pred, axis=0).reset_index(drop=True)
    ...
    with torch.no_grad():
        ...
        valid_weight_pred_numpy = pred.data.cpu().numpy().reshape(-1, 1)
        valid_hop_pred_numpy = hop_pred.data.cpu().numpy().reshape(-1, 1)

        valid_weight_pred_df = pd.DataFrame(pred_numpy, columns=['w_pred'])
        valid_hop_pred_df = pd.DataFrame(hop_pred_numpy, columns=['h_pred'])

        # valid_weight_pred.append(valid_weight_pred_df)
        # valid_hop_pred.append(valid_hop_pred_df)
    ...
    all_weight_pred_df = pd.concat([train_weight_pred_df, valid_weight_pred_df],axis=0).reset_index(drop=True)
    all_hop_pred_df = pd.concat([train_hop_pred_df, valid_hop_pred_df], axis=0).reset_index(drop=True)

    weight_df = pd.concat([edge_weights, all_weight_pred_df], axis=1)
    hop_df = pd.concat([hop_edge_dist, all_hop_pred_df], axis=1)

    weight_df.s = weight_df.s.astype(int)
    weight_df.t = weight_df.t.astype(int)
    weight_df.w_pred = weight_df.w_pred.round(2)
    weight_df.weight = weight_df.weight.round(2)

    hop_df.s = hop_df.s.astype(int)
    hop_df.t = hop_df.t.astype(int)
    hop_df.h_pred = hop_df.h_pred.round(2)

    weight_df.to_csv(os.path.join(pred_path, f'weight_dist_{pre_epoch}.csv'), sep=',', header=True, index=False)
    hop_df.to_csv(os.path.join(pred_path, f'hop_dist_{pre_epoch}.csv'), sep=',', header=True, index=False)
...
#     all_weight_pred = []
#     all_hop_pred = []
#     for batch in range(num_all_batch):
#         X_emb = copy.copy(X)
#         for tier in range(args.tier):
#             X_emb = gnn(X_emb, edge_index[tier])
#         print('training epoch:{epoch}, batch:{batch}:'.format(epoch=str(pre_epoch), batch=str(batch)))
...
#         print('X_emb:', X_emb[:5])
#
#         all_batch_s = edge_weights['s'].values[batch * batch_size:(batch + 1) * batch_size]
#         all_batch_t = edge_weights['t'].values[batch * batch_size:(batch + 1) * batch_size]
#         pred = regressor([X_emb[all_batch_s], X_emb[all_batch_t]])
#
#         hop_all_batch_s = hop_edge_dist['s'].values[batch * batch_size:(batch + 1) * batch_size]
#         hop_all_batch_t = hop_edge_dist['t'].values[batch * batch_size:(batch + 1) * batch_size]
#         hop_pred = regressor_2([X_emb[hop_all_batch_s], X_emb[hop_all_batch_t]])
#
#         pred_numpy = pred.data.cpu().numpy().reshape(-1, 1)
#         hop_pred_numpy = hop_pred.data.cpu().numpy().reshape(-1, 1)
#
...
#         hop_pred_df = pd.DataFrame(hop_pred_numpy, columns=['h_pred'])
#
#         all_weight_pred.append(pred_df)
#         all_hop_pred.append(hop_pred_df)
#
#     pred_df = pd.concat(all_weight_pred, axis=0).reset_index(drop=True)
#     hop_pred_df = pd.concat(all_hop_pred, axis=0).reset_index(drop=True)
#
...
#     hop_df.h_pred = hop_df.h_pred.round(2)
#
#     weight_df.to_csv(os.path.join(pred_path, f'weight_dist_{args.node}.csv'), sep=',', header=True, index=False)
#     hop_df.to_csv(os.path.join(pred_path, f'hop_dist_{args.node}.csv'), sep=',', header=True, index=False)


end = datetime.now()
print('end time:', end)

elapsed_time = (end-start).total_seconds()/60
