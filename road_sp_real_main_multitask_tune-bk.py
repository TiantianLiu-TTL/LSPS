...
def load_data():
    zero_edge_index = pd.read_csv('./data/road-NA/edge_3hop2tier_tier_0.txt', sep='\t', names=['s', 't']).T.astype(int)
    one_edge_index = pd.read_csv('./data/road-NA/edge_3hop2tier_tier_1.txt', sep='\t', names=['s', 't']).T.astype(int)
    two_edge_index = pd.read_csv('./data/road-NA/edge_3hop2tier_tier_2.txt', sep='\t', names=['s', 't']).T
    edge_weights = pd.read_csv('./data/road-NA/whole_edge_dist.csv', sep=',', header=0).astype(float)
    hop_edge_dist = pd.read_csv('./data/road-NA/whole_edge_hop_dist.csv', sep=',', header=0).astype(float)
    # edge_weights_0218 = pd.read_csv('./data/road-NA/samples_0218/whole_edge_dist_0218.csv', sep=',', header=0).astype(float)
    # hop_edge_dist_0218 = pd.read_csv('./data/road-NA/samples_0218/whole_edge_hop_dist_0218.csv', sep=',', header=0).astype(float)
    sa_edge_weights = pd.read_csv('./data/road-NA/whole_edge_dist_sa.csv', sep=',', header=0).astype(float)
    sa_hop_edge_dist = pd.read_csv('./data/road-NA/whole_edge_hop_dist_sa.csv', sep=',', header=0).astype(float)

    # edge_weights = pd.concat([edge_weights, sa_edge_weights], axis=0).sample(frac=1).reset_index(
    #     drop=True)
    # hop_edge_dist = pd.concat([hop_edge_dist, sa_hop_edge_dist], axis=0).sample(frac=1).reset_index(
    #     drop=True)

    edge_weights = pd.concat([edge_weights, sa_edge_weights], axis=0).reset_index(
        drop=True)
    hop_edge_dist = pd.concat([hop_edge_dist, sa_hop_edge_dist], axis=0).reset_index(
        drop=True)

    # edge_weights = pd.concat([edge_weights, edge_weights_0218, sa_edge_weights], axis=0).sample(frac=1).reset_index(drop=True)
    # hop_edge_dist = pd.concat([hop_edge_dist, hop_edge_dist_0218, sa_hop_edge_dist], axis=0).sample(frac=1).reset_index(drop=True)

    node_feats = pd.read_csv('./data/road-NA/feature_3hop2tier.txt', sep='\t',header=None, index_col=0).astype(float)
    print('node feat null:', pd.isnull(node_feats).sum())
    print('node null samples location:', np.where(node_feats.isna().values == 1))
    null_values = node_feats.values[np.where(node_feats.isna().values == 1)]
    print('node null samples:', null_values)
...
edge_weight_shp = edge_weights.shape
hop_edge_shp = hop_edge_dist.shape

if edge_weight_shp[0] > hop_edge_shp[0]:
    edge_weights = edge_weights.iloc[:hop_edge_shp[0], :]
    print(111111)
else:
    hop_edge_dist = hop_edge_dist.iloc[:edge_weight_shp[0], :]
    print(222222)

print('edge_weights shape', edge_weights.shape)
print('hop_edge_dist shape', hop_edge_dist.shape)
...
# hop_edge_dist = hop_edge_dist.loc[:,:].reset_index()

print('edge_weights:', edge_weights.reset_index(drop=True).loc[:5,:])

print('hop_edge_dist1111', hop_edge_dist.shape)
print('edge_weights2222', edge_weights.shape)
...
for pre_epoch in range(pretrain_epochs):
    for batch in range(num_train_batch):
        ...
        opt.zero_grad()
        loss = weight_loss + 1000*hop_loss
        loss.backward()
        opt.step()
    ...
    with torch.no_grad():
        ...
        weight_valid_mape = torch.sum(torch.abs(pred - ori_weight_valid_weights) / ori_weight_valid_weights) / pred.shape[0]
        weight_mape_error.append(weight_valid_mape.item())

        hop_valid_mape = torch.sum(torch.abs(hop_pred - ori_hop_valid_weights) / ori_hop_valid_weights) / hop_pred.shape[0]
    ...
    #         print('valid epoch:{epoch}, batch:{batch}:'.format(epoch=str(pre_epoch), batch=str(batch)))
    #         valid_batch_s = valid_df['s'].values[batch*batch_size:(batch+1)*batch_size]
    #         valid_batch_t = valid_df['t'].values[batch*batch_size:(batch+1)*batch_size]
    #         pred = regressor([X_emb[valid_batch_s], X_emb[valid_batch_t]])
    #
    #         hop_valid_batch_s = hop_valid_df['s'].values[batch*batch_size:(batch+1)*batch_size]
    #         hop_valid_batch_t = hop_valid_df['t'].values[batch*batch_size:(batch+1)*batch_size]
    #         hop_pred = regressor_2([X_emb[hop_valid_batch_s], X_emb[hop_valid_batch_t]])
    #
    #         print('before:', ori_weight_valid_weights.shape)
    #         valid_weights = ori_weight_valid_weights[batch*batch_size:(batch+1)*batch_size]
    #         print('after:', valid_weights.shape)
    #         print('num_valid_batch:',num_valid_batch)
    #         print('valid pred', pred[:10])
    #         print('valid true:', valid_weights[:10])
    #
    #         weight_valid_loss = F.mse_loss(pred, valid_weights) / pred.shape[0]
    #         print('valid weight loss:', weight_valid_loss.item())
    #         weight_valid_rmse_error.append(torch.sqrt(weight_valid_loss).item())
    #
    #         hop_valid_weights = ori_hop_valid_weights[batch*batch_size:(batch+1)*batch_size]
    #
    #         print('valid hop pred', hop_pred[:10])
