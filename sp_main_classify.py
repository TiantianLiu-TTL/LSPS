...
def load_data():
    zero_edge_index = pd.read_csv('./data/edge_tier_0.txt', sep='\t', names=['s', 't']).T
    one_edge_index = pd.read_csv('./data/edge_tier_1.txt', sep='\t', names=['s', 't']).T
    two_edge_index = pd.read_csv('./data/edge_tier_2.txt', sep='\t', names=['s', 't']).T
    edge_weights = pd.read_csv('./data/whole_node_pair_samples.csv', sep=',', header=0)
    edge_weights = edge_weights.sample(frac=1).head(int(edge_weights.shape[0]/2))
    ...
    # print(node_feats[:5])
    return zero_edge_index, one_edge_index, two_edge_index, edge_weights, node_feats

zero_edge_index, one_edge_index, two_edge_index, edge_weights, node_feats = load_data()
...
train_labels = torch.FloatTensor(train_df[['label']].values).to(device)
valid_df = edge_weights.iloc[int(shp[0] * 0.8):, :]
valid_labels = torch.FloatTensor(valid_df[['label']].values).to(device)
...
regressor = MLPNet(out_channels_3*3, final_out_dim).to(device)
...
for pre_epoch in range(pretrain_epochs):
    ...
    pred = regressor([X_emb[train_df['s'].values], X_emb[train_df['t'].values], X_emb[train_df['c'].values]])
    print('pretrain pred shape', pred.shape)
    print('pred', pred[:10])
    print('true:', train_labels[:10])
    # loss = F.mse_loss(train_weights, pred)/1000

    loss = F.binary_cross_entropy(pred, train_labels)
    loss.backward()
    opt.step()
    pre_train_loss = loss.item()
    print('{n} epoch pretrain loss:'.format(n=pre_epoch), pre_train_loss)
    pre_train_losses.append(pre_train_loss)
    gnn.eval()
    regressor.eval()
    with torch.no_grad():
        print('evaluation')
        pred = regressor([X_emb[valid_df['s'].values], X_emb[valid_df['t'].values], X_emb[valid_df['c'].values]])
        print('valid pred', pred[:10])
        print('valid true:', valid_labels[:10])
        valid_loss = F.binary_cross_entropy(pred, valid_labels)
