...
# if edge_weights.shape[0]%batch_size == 0:++__
...
for pre_epoch in range(pretrain_epochs):
    ...
    with torch.no_grad():
        ...
        valid_weight_pred_df = pd.DataFrame(valid_weight_pred_numpy, columns=['w_pred'])
        valid_hop_pred_df = pd.DataFrame(valid_hop_pred_numpy, columns=['h_pred'])
        ...
        weight_valid_loss = F.mse_loss(pred, ori_weight_valid_weights)
        ...
        hop_valid_loss = F.mse_loss(hop_pred, ori_hop_valid_weights)
    ...
    if pre_epoch < (pretrain_epochs-1):
        continue

    print('train_weight_pred_df', train_weight_pred_df.shape)
    print('valid_weight_pred_df', valid_weight_pred_df.shape)
    print('train_hop_pred_df', train_hop_pred_df.shape)
    print('valid_hop_pred_df', valid_hop_pred_df.shape)
    ...
    print('edge_weights, hop_edge_dist', edge_weights.shape, hop_edge_dist.shape)
