...
def load_data():
    ...
    edge_weights = pd.read_csv('./data/whole_edge_dist.csv', sep=',', header=0)
    hop_edge_dist = pd.read_csv('./data/whole_edge_hop_dist.csv', sep=',', header=0)
...
groud_truth_edge_dist.to_csv('./data/edge_dist_ground_truth.csv', index=False, header=True)
...
for pre_epoch in range(pretrain_epochs):
    ...
    loss = F.mse_loss(pred, train_weights)/1000
    hop_loss = F.mse_loss(hop_pred, hop_train_weights)/1000
